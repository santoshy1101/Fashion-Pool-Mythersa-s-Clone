            ...
            <Carousel>
                <Carousel.Item>
                    <Flex justifyContent='space-between' alignItems='center'>                        {
                        products.filter(item => item.category === category).filter((item, i) => (i >= 12 && i <= 15)).map((item, i) => (
                ...
                <Carousel.Item>
                    <Flex justifyContent='space-between' alignItems='center'>
                        {
                            products.filter(item => item.category === category).filter((item, i) => (i >= 16 && i <= 19)).map((item, i) => (
                                <Box key={item.id} w='20%'>
                                    <Link to={`/${item.id}`}>
                                        <Image src={item.image} alt='someimg' />
                                    </Link>                                    <Stack spacing='0' alignItems='center' justifyContent='center' mt={1}>
